...
impl<Pane> Tree<Pane> {
    ...
    pub fn simplify(&mut self, options: &SimplificationOptions) {
        ...
        if options.all_panes_must_have_tabs {
            ...
            // Same for every detached viewport: if its root was a bare pane it is now wrapped
            // in a fresh tab container, and the viewport must follow that new root id —
            // otherwise the window would keep pointing at a tile that became a child.
            let mut viewports = std::mem::take(&mut self.viewport_tiles);
            for vp in &mut viewports {
                if let Some(new_root) = self.tiles.make_all_panes_children_of_tabs(false, vp.root) {
                    vp.root = new_root;
                }
            }
            self.viewport_tiles = viewports;
